                        ...
                        foreach ($response as $key => $value) {
                            ...
                            switch ($data['FieldType']) {
                                ...
                                '<input type="checkbox" class="checkBoxClass" name="mycheckbox[]" id="field_' . 2 . '" value="CheckBox' . 2 . '"/>' . '<input type="text" style="width:160px;height:25px" name="checkboxcaption[]" id="' . $key++ . '" value=""/>' . '<button class="removeclass3">x</button>' . '<button class="addclass3">+</button>' . '</p>' . '</div>';
                                    break;
                                case "Combobox":
                                    echo '<div class="name box-style" id="InputsWrapper_1' . 1 . '">' .
                                    '<select class="selectBox" style="width:183px;height:30px" id=' . $key . '>';
                                    foreach ($data['ListValues'] as $value) {
                                        echo "<option value=" . $value . ">" . $value . "</option>";
                                    }
                                    echo '<input type="hidden"  name="myCombo[]" id="cb' . $key . '" value=""/>' . '<button class="removeclassCombo">x</button>' .
...
//        return false;
    ...
    $(document).ready(function () {
        ...
        $("body").on("click", ".addclassCombo", function (event) {
            ...
            var total = numItemsSelect + numItemsText + numItemsCheck;
            //alert(total);
            $(this).parent('div').after('<div class="name box-style" id="InputsWrapper_1' + 1 + '">' +
                    '<select class="selectBox" style="width:183px;height:30px" id=' + total + '>' + '<option value="' + total + '"' + '>' + total + '</option>' +
                    '<input type="hidden"  name="myCombo[]" id="cb' + total + '" value=""/>' + '<button class="removeclassCombo">x</button>' +
